...
namespace Log {
	...
	void writeLexicalTable(const LOG& log, LT::LexTable& Lextable, IT::IdTable& idt)
	{
		bool r = false;
		int x = 0,n=0, e = NULL,w=NULL;
		*log.stream << "\nТаблица лексем:\n";
		...
		for (int i = 0; i < Lextable.size; i++)
		{
			...
			case LEX_RIGHTTHESIS: {
			  ...
			  if (e != NULL) {
				  if (n > 1)
					  if (idt.table[e].iddatatype==idt.table[w].iddatatype)
						  throw ERROR_THROW_IN(124, Lextable.table[i - 1].sn, NULL);
			  }
			  break;
			}
		}
	...
	int k = 0, key = 0;
	LT::LIT lit;
	void writeAllTokens(const LOG& log, IT::IdTable& idt,LT::LexTable& Lextable, In::IN& In)
	{
		*log.stream << "\nТаблица слов: \n" <<
		...
		for (int i = 0; i < In.TokenCount; i++)
		{
			...
			*log.stream << std::setw(6) << std::left << i <<
			...
			if (In.tokens[i].isLiteral) // ("Да") : ("Нет")) <<
			{
				...
				if (key == 0)
				{
					...
					if (In.tokens[i].token[0] == '"')
						lit.typel[k] = LEX_TYPE_STRING;
					else {
						lit.typel[k] = LEX_TYPE_INT;
					}
					k++;
				}
				*log.stream <<
					std::setw(10) << std::left << ("YES");
			}
			else {
				*log.stream <<
			...
			*log.stream << 
				std::setw(10) << std::left << idt.table[i].pref<<
				std::setw(30) << std::left << In.tokens[i].token <<
				std::endl;
		...
		*log.stream << "\nТаблица идентификаторов: \n" <<
		...
		for (int i = 0; i < IDtable.size; i++)
		{
			*log.stream << std::setw(5) << std::left << i <<
			...
			if (IDtable.table[i].iddatatype == IT::CR)
				...
				*log.stream << std::endl;
				...
				if (IDtable.table[i].iddatatype == IT::STR) {
					...
					*log.stream << std::endl;
						...
						if (IDtable.table[i].iddatatype == IT::CR) {
							*log.stream << ((IDtable.table[i].value.vcr.len > 0) ? (IDtable.table[i].value.vcr.cr) : (""));
							*log.stream << std::endl;
						}
		}
	}
